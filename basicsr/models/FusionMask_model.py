...
class FusionMaskModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('FrequencyDomainFusionLoss_opt'):
            self.cri_FrequencyDomain = build_loss(train_opt['FrequencyDomainFusionLoss_opt']).to(self.device)
        else:
            self.cri_FrequencyDomain = None
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.cri_FrequencyDomain:
            l_FrequencyDomain = self.cri_FrequencyDomain(self.vi_ycrcb, self.ir, self.output_ycrcb)
            l_total += l_FrequencyDomain
            loss_dict['l_FrequencyDomain'] = l_FrequencyDomain
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        ####
        iter_visualization_path = osp.join(self.opt['path']['visualization'], str(current_iter))
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            total_time += elapsed
            logger.info(f"[{idx + 1}/{len(dataloader)}] 推理时间: {elapsed:.2f} ms")
            ...
            if save_img:
                if self.opt['is_train']:
                    save_img_path_vi = osp.join(iter_visualization_path,
                                                f'{img_name}_{current_iter}_vi.png')
                    save_img_path_ir = osp.join(iter_visualization_path,

                                                f'{img_name}_{current_iter}_ir.png')
                    save_img_path_mask = osp.join(iter_visualization_path,
                                                  f'{img_name}_{current_iter}_mask.png')
                    save_img_path_fused = osp.join(iter_visualization_path,
                                                   f'{img_name}.png')
                else:
                    if self.opt['val']['suffix']:
                        save_img_path_vi = osp.join(iter_visualization_path,
        ...
        if total_images > 0:
            ...
            logger.info(f"🔥 数据集 {dataset_name} 平均推理耗时: {avg_time:.2f} ms/图像")
