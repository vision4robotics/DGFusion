...
@LOSS_REGISTRY.register()
class FrequencyDomainFusionLoss(nn.Module):
    def __init__(self, loss_weight=1.0, reduction='mean'):
        super(FrequencyDomainFusionLoss, self).__init__()
        self.loss_weight = loss_weight
        self.reduction = reduction

    def fft2(self, x):
        """ 计算2D的FFT，返回频域的复数（实部和虚部）"""
        return torch.fft.fft2(x)

    def get_amplitude(self, complex_tensor):
        """ 从复数频域中提取幅度 """
        return torch.abs(complex_tensor)

    def forward(self, image_vis, image_ir, generate_img):
        """
        计算频域损失：融合图像与可见光和红外图像的频域差值的平方误差。
        :param image_vis: 可见光图像 [B, C, H, W]
        :param image_ir: 红外图像 [B, C, H, W]
        :param generate_img: 融合图像 [B, C, H, W]
        :return: 返回平方误差损失
        """
        # 计算频域：FFT
        vis_freq = self.fft2(image_vis)
        ir_freq = self.fft2(image_ir)
        fused_freq = self.fft2(generate_img)

        # 提取幅度（可以选择实部或幅度，推荐使用幅度来避免计算复杂性问题）
        vis_amp = self.get_amplitude(vis_freq)
        ir_amp = self.get_amplitude(ir_freq)
        fused_amp = self.get_amplitude(fused_freq)

        # 计算幅度差的平方误差
        # 目标是要最小化 (fused_amp - (vis_amp + ir_amp)) ^ 2
        amplitude_diff = fused_amp - (vis_amp + ir_amp)/2

        # 计算平方误差
        if self.reduction == 'mean':
            loss = torch.mean(amplitude_diff ** 2)
        elif self.reduction == 'sum':
            loss = torch.sum(amplitude_diff ** 2)
        else:
            loss = amplitude_diff ** 2  # No reduction (element-wise loss)
